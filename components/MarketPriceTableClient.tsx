...
/** Mesma UI/interacção do MarketPriceTable original, mas recebe os dados já
 * prontos (buscados no servidor) em vez de os ir buscar no browser depois
 * de a página aparecer — evita o atraso "aparece vazio, só depois enche". */
export function MarketPriceTableClient({ initialData }: { initialData: MarketItem[] }) {
    const [activeCategory, setActiveCategory] = useState("Todos");
    const [searchTerm, setSearchTerm] = useState("");
    const [selectedProvince, setSelectedProvince] = useState("Todas");

    const lastUpdated = new Date().toLocaleDateString('pt-MZ', { day: 'numeric', month: 'long', year: 'numeric' });
    const marketData = initialData;
    ...
    return (
        <div className="w-full bg-white rounded-[10px] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-200 overflow-hidden">
            {/* Header */}
