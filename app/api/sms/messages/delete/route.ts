...
// Eliminar SMS do painel. Só admin.
//  mode "soft" (default): marca deleted_at -> vai para o separador "Eliminadas".
//  mode "hard": apaga mesmo (só usar a partir de "Eliminadas").
//  mode "restore": tira do "Eliminadas".
...
export async function POST(request: Request) {
    ...
    try {
        ...
        if (mode === "hard") {
            const { error } = await admin.from("sms_messages").delete().in("id", ids);
        ...
        console.error("sms/messages/delete error:", err);
