...
export default function LixoPage() {
    ...
    const [confirmAction, setConfirmAction] = useState<null | { kind: "restore" | "delete" | "empty"; ids: string[] }>(null);
        ...
        try {
            if (kind === "restore") {
                const res = await fetch("/api/admin/articles", {
                    ...
                    body: JSON.stringify({ ids, payload: { deleted_at: null } }),
                });
                if (!res.ok) throw new Error((await res.json()).error);
                toast.success(`${ids.length} notícia(s) restaurada(s).`);
            } else {
                const res = await fetch("/api/admin/articles", {
                    method: "DELETE",
                    headers: { "Content-Type": "application/json" },
                ...
                if (!res.ok) throw new Error((await res.json()).error);
                toast.success(`${ids.length} notícia(s) eliminada(s) permanentemente.`);
            }
        ...
        <div className="text-[#2c3338]">
            ...
            {selectedIds.size > 0 && (
                <div className="flex items-center gap-3 mb-4 p-3 bg-blue-50 border border-blue-200 rounded-md">
                    ...
                    <button
                        onClick={() => setConfirmAction({ kind: "delete", ids: Array.from(selectedIds) })}
                            ...
                            <tr key={item.id} className={`group border-b border-[#f0f0f1] hover:bg-[#f6f7f7] ${idx % 2 === 0 ? "bg-white" : "bg-[#f9f9f9]"} ${selectedIds.has(item.id) ? "bg-red-50" : ""}`}>
                                ...
                                <td className="p-3 text-center">
                                    <div className="flex items-center justify-center gap-2">
                                        ...
                                        <button
                                            onClick={() => setConfirmAction({ kind: "delete", ids: [item.id] })}
            ...
            <ConfirmationModal
                ...
                title={confirmAction?.kind === "restore" ? "Restaurar Notícias" : "Eliminar Permanentemente"}
                description={
                    confirmAction?.kind === "restore"
                        ? `Restaurar ${confirmAction.ids.length} notícia(s)?`
                        : `Eliminar permanentemente ${confirmAction?.ids.length} notícia(s)? Esta acção não pode ser revertida.`
                }
